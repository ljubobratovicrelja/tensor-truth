...
def convert_pdf_to_markdown(pdf_path, preserve_math=True, converter='pymupdf'):
    ...
    try:
        ...
        if preserve_math and md_text and "[PDF content extraction failed" not in md_text:
...
def convert_with_marker(pdf_path):
    ...
    try:
        if MARKER_CONVERTER is None:
            ...
            logging.info(f"Loading Marker models on {device.upper()} (One-time setup)...")
...
def fetch_and_convert_paper(category, arxiv_id, skip_if_exists=True, converter='pymupdf'):
...
def fetch_and_convert_book(category, book_item, skip_if_exists=True, converter='pymupdf'):
    """
...
def rebuild_category(config, category, config_path, workers=1, converter='pymupdf'):
    ...
    if category_type == 'papers':
        def process_item(paper):
            ...
            try:
                result = fetch_and_convert_paper(category, arxiv_id, skip_if_exists=True, converter=category_converter)
                if result is None:
        ...
        def process_item(book):
            ...
            try:
                result = fetch_and_convert_book(category, book, skip_if_exists=True, converter=category_converter)
                if result is None:
                    stats.increment('skipped')
...
Books are defined in the JSON config with the following structure:
  {
    ...
    if args.rebuild_all:
        ...
        logging.info(f"Rebuilding all {len(categories)} categories with {args.workers} workers (converter: {args.converter})")
        
        for category in categories:
            rebuild_category(config, category, args.config, workers=args.workers, converter=args.converter)
        return
