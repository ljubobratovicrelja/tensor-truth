...
def __getattr__(name):
    ...
    # Helpers
    if name in ("download_indexes_with_ui", "ensure_engine_loaded", "free_memory",
                "get_available_modules", "get_ollama_models", "get_system_devices"):
        from .helpers import (
            download_indexes_with_ui,
            ensure_engine_loaded,
            free_memory,
    ...
    # Session
    if name in ("create_session", "load_sessions", "rename_session",
                "save_sessions", "update_title"):
        from .session import create_session, load_sessions, rename_session, save_sessions, update_title
        return locals()[name]
