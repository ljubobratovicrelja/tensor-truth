...
def _handle_simple_llm_mode(
    session: dict, params: dict, current_id: str
) -> Tuple[str, dict]:
    ...
    try:
        ...
        if engine:
            thinking, message_data = _handle_rag_mode(
                engine, prompt, params, modules, has_pdf_index, current_id
            )
        else:
            thinking, message_data = _handle_simple_llm_mode(
                session, params, current_id
            )
