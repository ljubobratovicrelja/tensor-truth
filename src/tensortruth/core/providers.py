...
class ProviderRegistry:
    ...
    def _get_llama_cpp_models(self, provider) -> List[ModelReference]:
        ...
        try:
            ...
            logger.warning("Failed to query llama.cpp models for %s: %s", provider.id, e)
    ...
    def resolve_model(
        ...
        if provider_id:
            ...
            if provider is None:
                ...
                logger.warning("Provider '%s' not found, falling back to default", provider_id)
                ...
                try:
                    ...
                    for sm in get_available_models(base):
                        if sm.get("id") == model_name:
                            return self._resolve_for_provider(
                                model_name, provider
                            )
                except Exception:
                    pass
    ...
    def _resolve_for_provider(self, model_name: str, provider) -> ModelReference:
        ...
        if provider.type == "ollama":
            ...
            cached_caps = self._caps_cache.get(caps_key, [])
            # Try to find model in server's model list
            server_models = get_available_models(base)
            for sm in server_models:
                if sm.get("id") == model_name:
                    ...
                    for m in provider.models:
                        if m.get("name") == model_name:
                            static = m
                            break
                    caps = static.get("capabilities", []) or cached_caps
                    return ModelReference(
                        provider_id=provider.id,
                        model_name=model_name,
            ...
            # Model not found on server — construct best-effort ref
            static = {}
            for m in provider.models:
                if m.get("name") == model_name:
                    static = m
    ...
    def _fallback_model_ref(
        self, model_name: str, provider_id: str
    ) -> ModelReference:
    ...
    def check_tool_support(self, model_ref: ModelReference) -> bool:
        ...
        if model_ref.provider_type == "llama_cpp":
            return self._check_llama_cpp_capability(
                model_ref, "tools"
            )
        return False
    ...
    def _check_llama_cpp_capability(
        self, model_ref: ModelReference, cap: str
    ) -> bool:
        ...
        if key in self._caps_cache:
            ...
            caps = check_capabilities(
                model_ref.base_url, model_ref.model_name
            )
...
def _get_ollama_model_info(base_url: str, model_name: str) -> Dict[str, Any]:
    """Query ``/api/show`` for a single Ollama model. Results are cached."""
    import requests
    ...
    try:
        resp = requests.post(
            f"{base_url}/api/show", json={"model": model_name}, timeout=2
        )
        if resp.status_code == 200:
            data = resp.json()
...
def _create_openai_like_llm(model_ref: ModelReference, **kwargs) -> LLM:
    ...
    max_tokens = kwargs.pop(
        "max_tokens", additional.get("num_predict", None)
    )
...
def get_orchestrator_llm(model_ref: ModelReference, context_window: int) -> LLM:
    ...
    key = (model_ref.provider_id, model_ref.model_name, model_ref.base_url, context_window)
...
def get_tool_llm(
    ...
    resolved_thinking = thinking if thinking is not None else (
        ProviderRegistry.get_instance().check_thinking_support(model_ref)
    )
