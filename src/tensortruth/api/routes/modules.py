        ...
        if provider.type == "ollama":
            ...
            try:
                ...
                for sm in server_models:
                    ...
                    # Dynamic capability detection when no static config
                    if not caps:
                        caps_key = f"{provider.id}:{model_id}"
                        cached = registry._caps_cache.get(caps_key)
                        if cached is not None:
                            ...
                            registry._caps_cache[caps_key] = caps
                    models.append(
                        ModelInfo(
                            name=model_id,
                            provider_id=provider.id,
