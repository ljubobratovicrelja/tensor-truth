...
def _is_orchestrator_enabled(
    ...
    try:
        model_ref = resolve_model_from_params(params, model_name)
        has_tools = ProviderRegistry.get_instance().check_tool_support(model_ref)
    except Exception:
        logger.warning(
...
async def _run_orchestrator_path(
    ...
    if provider_id:
        model_ref = resolve_model_from_params(params, model_name or "")
                ...
                if command:
                    ...
                    if needs_title and full_response:
                        try:
                            ...
                            cmd_provider = session.get("params", {}).get("provider_id")
                            title = await generate_smart_title_async(
                                full_response,
                                cmd_model,
                                cmd_ctx,
