...
_CODE_EXECUTION_RULES = (
    "CODE EXECUTION:\n"
    "   - Python code blocks will be AUTOMATICALLY EXECUTED in a "
    "persistent container session.\n"
    "   - Variables, imports, and state PERSIST across ALL code blocks in "
    "this conversation (like Jupyter notebooks).\n"
    "   - You can reference variables/functions defined in ANY previous "
    "code block in the Chat History.\n"
    "   - Example: If you defined `df = pd.DataFrame(...)` earlier, you "
    "can use `df` in new code blocks.\n"
    "   - Review the Chat History to see what variables/imports are already "
    "available before redefining them.\n"
    '   - Include `if __name__ == "__main__":` blocks for executable '
    "examples.\n"
    "   - Make code print informative output - the output will be shown "
    "directly to the user.\n"
    "   - DO NOT predict or describe the output - the actual execution "
    "result will be displayed.\n"
    "   - MATPLOTLIB PLOTS: Just use plt.figure() and plt.plot() - figures "
    "are AUTO-SAVED after execution.\n"
    "   - You do NOT need to call plt.savefig() or plt.show() - ALL open "
    "figures are saved automatically.\n"
    "   - If you want to save with a specific filename, you CAN still use "
    "plt.savefig('custom_name.png').\n"
    "   - You can save files anywhere (subdirectories OK): plots/chart.png, "
    "outputs/data.csv, etc.\n"
    "   - ALL new/modified image files (.png, .jpg, .svg, .pdf) are "
    "automatically displayed.\n"
    "   - ALL new/modified data files (.csv, .json, .txt, .html) are "
...
CUSTOM_CONTEXT_PROMPT_NO_SOURCES = (
    ...
    "(e.g., 'what did you do', 'explain that', 'fix the error'), answer using the "
    "conversation history. Do NOT mention missing documents.\n"
