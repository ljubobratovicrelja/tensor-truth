...
async def web_search_async(
    ...
    response = (
        ...
        f"### Sources\n{sources_section}\n\n"
        f"---\n"
        f"💡 *Web search summary generated by {model_name}. "
        f"Successfully fetched {len(pages)}/{len(all_attempts)} pages. "
        f"For detailed documentation, use loaded knowledge bases.*"
    )
