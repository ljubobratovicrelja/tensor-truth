"""Unit tests for document index builder."""
...
@pytest.fixture
def document_builder(temp_session_dirs):
    """Create DocumentIndexBuilder with explicit dirs."""
    return DocumentIndexBuilder(
        index_dir=temp_session_dirs["index"],
...
class TestDocumentIndexBuilderInit:
    """Test DocumentIndexBuilder initialization."""

    def test_sets_directory_paths(self, document_builder, temp_session_dirs):
        """Should set correct directory paths."""
        assert document_builder.index_dir == temp_session_dirs["index"]
        assert document_builder.markdown_dir == temp_session_dirs["markdown"]

    def test_creates_dirs_if_missing(self, tmp_path):
        """Should create directories if they don't exist."""
        index_dir = tmp_path / "new_index"
        markdown_dir = tmp_path / "new_markdown"
        builder = DocumentIndexBuilder(index_dir=index_dir, markdown_dir=markdown_dir)
        assert builder.index_dir.exists()
        assert builder.markdown_dir.exists()


class TestIndexExists:
    """Test index existence check."""

    def test_returns_false_when_no_index(self, document_builder):
        """Should return False if index doesn't exist."""
        # Clear index directory
        if document_builder.index_dir.exists():
            shutil.rmtree(document_builder.index_dir)
        document_builder.index_dir.mkdir(parents=True)

        assert not document_builder.index_exists()

    def test_returns_false_without_chroma_db(self, document_builder):
        """Should return False if chroma.sqlite3 missing."""
        # Create only docstore
        (document_builder.index_dir / "docstore.json").write_text("{}")

        assert not document_builder.index_exists()

    def test_returns_false_without_docstore(self, document_builder):
        """Should return False if docstore.json missing."""
        # Create only chroma db
        (document_builder.index_dir / "chroma.sqlite3").write_text("")

        assert not document_builder.index_exists()

    def test_returns_true_with_both_files(self, document_builder):
        """Should return True if both required files exist."""
        (document_builder.index_dir / "chroma.sqlite3").write_text("")
        (document_builder.index_dir / "docstore.json").write_text("{}")

        assert document_builder.index_exists()


class TestBuildIndex:
    """Test index building."""

    @patch("tensortruth.document_index.StorageContext")
    ...
    @patch("tensortruth.document_index.get_leaf_nodes")
    @patch("tensortruth.document_index.HierarchicalNodeParser")
    @patch("tensortruth.document_index.SimpleDirectoryReader")
    def test_loads_markdown_files(
        ...
        document_builder.build_index(sample_markdown_files)
    ...
    def test_uses_hierarchical_chunking(
        ...
        document_builder.build_index(
            sample_markdown_files, chunk_sizes=[2048, 512, 128]
        )
    ...
    @patch("tensortruth.document_index.SimpleDirectoryReader")
    def test_raises_on_no_markdown_files(self, mock_reader, document_builder):
    ...
    def test_cleans_existing_index(
        ...
        old_file = document_builder.index_dir / "old_data.json"
        old_file.write_text("{}")
    ...
    def test_uses_default_chunk_sizes(self, document_builder, sample_markdown_files):
        """Should use default chunk sizes if none provided."""
        with (
            patch("tensortruth.document_index.StorageContext") as mock_storage,
            patch("tensortruth.document_index.HierarchicalNodeParser") as mock_parser,
            patch("tensortruth.document_index.SimpleDirectoryReader") as mock_reader,
            patch("tensortruth.document_index.get_leaf_nodes") as mock_get_leaf,
            patch("tensortruth.document_index.VectorStoreIndex"),
            patch("tensortruth.document_index.get_embed_model"),
            patch("tensortruth.document_index.chromadb.PersistentClient"),
        ):
...
class TestRebuildIndex:
    ...
    @patch.object(DocumentIndexBuilder, "build_index")
    def test_calls_build_with_no_files(self, mock_build, document_builder):
        """Should call build_index with None for markdown_files."""
        document_builder.rebuild_index()

        mock_build.assert_called_once_with(markdown_files=None, chunk_sizes=None)

    @patch.object(DocumentIndexBuilder, "build_index")
    def test_passes_chunk_sizes(self, mock_build, document_builder):
        """Should pass custom chunk sizes to build_index."""
        custom_sizes = [4096, 1024, 256]
        document_builder.rebuild_index(chunk_sizes=custom_sizes)
...
class TestDeleteIndex:
    ...
    def test_deletes_index_directory(self, document_builder):
        """Should remove entire index directory."""
        # Create some files in index
        (document_builder.index_dir / "chroma.sqlite3").write_text("")
        (document_builder.index_dir / "docstore.json").write_text("{}")
        assert document_builder.index_dir.exists()

        document_builder.delete_index()

        assert not document_builder.index_dir.exists()

    def test_handles_nonexistent_index(self, document_builder):
...
class TestGetIndexSize:
    ...
    def test_returns_zero_for_nonexistent_index(self, document_builder):
        """Should return 0 if index doesn't exist."""
        if document_builder.index_dir.exists():
            shutil.rmtree(document_builder.index_dir)

        assert document_builder.get_index_size() == 0

    def test_calculates_total_size(self, document_builder):
        """Should sum all file sizes in index directory."""
        # Create files with known sizes
        (document_builder.index_dir / "file1.txt").write_bytes(b"x" * 100)
        (document_builder.index_dir / "file2.txt").write_bytes(b"y" * 200)

        # Create subdirectory with file
        subdir = document_builder.index_dir / "subdir"
        subdir.mkdir()
        (subdir / "file3.txt").write_bytes(b"z" * 50)

        size = document_builder.get_index_size()
...
class TestGetDocumentCount:
    ...
    def test_returns_zero_for_no_documents(self, document_builder):
        """Should return 0 if no markdown files."""
        assert document_builder.get_document_count() == 0

    def test_counts_markdown_files(self, document_builder, sample_markdown_files):
        """Should count all .md files in markdown directory."""
        assert document_builder.get_document_count() == len(sample_markdown_files)

    def test_ignores_non_markdown_files(self, document_builder, sample_markdown_files):
        """Should only count .md files."""
        # Add a non-markdown file
        (document_builder.markdown_dir / "readme.txt").write_text("not markdown")
