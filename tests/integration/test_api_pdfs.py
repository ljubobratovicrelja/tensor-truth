...
class TestPDFsAPI:
    ...
    @pytest.mark.asyncio
    async def test_list_pdfs_session_not_found(self, client, mock_session_paths):
        """Test listing PDFs for non-existent session."""
        response = await client.get("/api/sessions/nonexistent/pdfs")
        assert response.status_code == 404

    @pytest.mark.asyncio
    async def test_list_pdfs_empty(self, client, mock_session_paths):
    ...
    @pytest.mark.asyncio
    async def test_delete_pdf_not_found(self, client, mock_session_paths):
    ...
    @pytest.mark.asyncio
    async def test_reindex_no_pdfs(self, client, mock_session_paths):
